...
internal class CachingMapBuilder<K, V> private constructor(
    ...
    private var keysView: CachingMapBuilderKeys<K>? = null
    private var valuesView: CachingMapBuilderValues<V>? = null
    private var entriesView: CachingMapBuilderEntries<K, V>? = null
    ...
    override val keys: MutableSet<K> get() {
        val cur = keysView
        return if (cur == null) {
            val new = CachingMapBuilderKeys(this)
            keysView = new
            new
        } else cur
    }
    ...
    override val values: MutableCollection<V> get() {
        val cur = valuesView
        return if (cur == null) {
            val new = CachingMapBuilderValues(this)
            valuesView = new
            new
        } else cur
    }
    ...
    override val entries: MutableSet<MutableMap.MutableEntry<K, V>> get() {
        val cur = entriesView
        return if (cur == null) {
            val new = CachingMapBuilderEntries(this)
            entriesView = new
            return new
        } else cur
    }
    ...
    private fun ensureCapacity(minCapacity: Int) {
        ...
        if (minCapacity > this.capacity) {
            val newSize = newCapacity(this.capacity, minCapacity)
    ...
    internal companion object {
        ...
        internal val Empty = CachingMapBuilder<Nothing, Nothing>(0).also { it.isReadOnly = true }
    ...
    internal open class Itr<K, V>(
        internal val map: CachingMapBuilder<K, V>
    ) {
    ...
    internal class KeysItr<K, V>(map: CachingMapBuilder<K, V>) : Itr<K, V>(map), MutableIterator<K> {
        override fun next(): K {
            checkForComodification()
    ...
    internal class ValuesItr<K, V>(map: CachingMapBuilder<K, V>) : Itr<K, V>(map), MutableIterator<V> {
        override fun next(): V {
            checkForComodification()
            if (index >= map.length) throw NoSuchElementException()
            lastIndex = index++
    ...
    internal class EntriesItr<K, V>(map: CachingMapBuilder<K, V>) : Itr<K, V>(map),
    ...
    internal class EntryRef<K, V>(
        private val map: CachingMapBuilder<K, V>,
        private val index: Int
    ) : MutableMap.MutableEntry<K, V> {
        private val expectedModCount = map.modCount

        override val key = map.keysArray[index]

        override var value = map.valuesArray!![index]
            private set
...
private class CachingMapBuilderKeys<E> internal constructor(
    private val backing: CachingMapBuilder<E, *>
...
private class CachingMapBuilderValues<V> internal constructor(
    val backing: CachingMapBuilder<*, V>
...
private abstract class AbstractCachingMapBuilderEntrySet<E : Map.Entry<K, V>, K, V> : AbstractMutableSet<E>() {
...
private class CachingMapBuilderEntries<K, V> internal constructor(
    val backing: CachingMapBuilder<K, V>
) : AbstractCachingMapBuilderEntrySet<MutableMap.MutableEntry<K, V>, K, V>() {
